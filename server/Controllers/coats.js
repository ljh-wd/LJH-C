...
const getAllCoats = async (req, res) => {
  ...
  let result = Coats.find(queryObject);
  if (sort) {
...
const getSingleCoat = async (req, res, next) => {
  const { id: coatID } = req.params;
  const singleCoat = await Coats.findById(coatID);
  if (!singleCoat) {
    return next(
      createCustomError(`Product with the id of ${coatID} doesn't exist`, 404)
    );
  }
  res.status(200).json({ singleCoat });
};

module.exports = { getAllCoats, getSingleCoat };
